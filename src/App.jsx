...
const TAX_RULES = {
  ...
  "Adobe CC":    { rate: 0.12, label: "VAT 12%", inclusive: true  },
  ...
  "Freepik":     { rate: 0.12, label: "VAT 12%", inclusive: true  },
  "CapCut":      { rate: 0,    label: "No VAT",  inclusive: false },
  "DaVinci":     { rate: 0,    label: "No VAT",  inclusive: false },
  ...
  "Netflix":     { rate: 0.12, label: "VAT 12%", inclusive: true  },
  "Spotify":     { rate: 0.12, label: "VAT 12%", inclusive: true  },
  "YouTube":     { rate: 0.12, label: "VAT 12%", inclusive: true  },
};
...
const AnimatedNum = ({ value }) => {
  ...
  return <span>{"₱" + disp.toLocaleString("en-PH")}</span>;
};

const INIT_EXPENSES = EXPENSE_CATS.reduce((a, c) => ({ ...a, [c]: { amount: "", currency: "PHP" } }), {});

export default function BudgetOS() {
  const [tab, setTab] = useSaved("bos_tab", "input");
  const [salary, setSalary] = useSaved("bos_salary", { amount: "", currency: "PHP" });
  const [extras, setExtras] = useSaved("bos_extras", []);
  const [savingsGoal, setSavingsGoal] = useSaved("bos_goal", "");
  const [expenses, setExpenses] = useSaved("bos_expenses", INIT_EXPENSES);
  const [subs, setSubs] = useSaved("bos_subs", []);
  const [history, setHistory] = useSaved("bos_history", []);
  const [runwayMonths, setRunwayMonths] = useSaved("bos_runway_months", 6);
  const [monthlySaveTarget, setMonthlySaveTarget] = useSaved("bos_save_target", 15000);
  ...
  const [usdRate, setUsdRate] = useState(57.5);
  const [rateStatus, setRateStatus] = useState("loading");

  useEffect(() => {
    fetch("https://open.er-api.com/v6/latest/USD")
      .then(r => r.json())
      .then(d => { if (d?.rates?.PHP) { setUsdRate(d.rates.PHP); setRateStatus("live"); } else setRateStatus("fallback"); })
  ...
  const genExpensesPHP = Object.values(expenses).reduce((s, e) => s + toPHP(e.amount, e.currency, usdRate), 0);
  ...
  const subRows = subs.map(s => {
    ...
    return { ...s, basePHP, taxRule, taxed: taxedTotal(basePHP, taxRule) };
  });
  ...
  const investTotal = subRows.filter(s => s.type === "Investment").reduce((s, x) => s + x.taxed.total, 0);
  const subExpTotal = subRows.filter(s => s.type === "Expense").reduce((s, x) => s + x.taxed.total, 0);
  ...
  function saveMonth() {
    ...
    setHistory(h => [{ month: m, income: totalIncome, spent: totalExpPHP, saved: Math.max(0, remaining), goal }, ...h]);
    setTab("dashboard");
  }
    ...
    <>
      <style>{`
        *,*::before,*::after{margin:0;padding:0;box-sizing:border-box;}
        ...
        .cta:hover{opacity:.82;}.cta:active{transform:scale(.98);}
        ...
        .cur-opt{padding:5px 9px;font-size:12px;font-weight:600;cursor:pointer;border:none;background:#fff;color:#aeaeb2;transition:background .13s,color .13s;}
          ...
          <div style={{ display: "flex", gap: 2, background: "#f0f0f0", padding: 3, borderRadius: 11 }}>
            {TABS.map(([k, l]) => (
              <button key={k} onClick={() => setTab(k)}
                ...
                style={{ padding: "5px 11px", border: "none", borderRadius: 8,
                  background: "transparent", color: "#6e6e73",
                  fontSize: 12, fontWeight: 500, fontFamily: SF }}>{l}</button>
            ))}
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: 5,
            background: "#f5f5f7", borderRadius: 20, padding: "4px 8px" }}>
            <div style={{ width: 6, height: 6, borderRadius: "50%",
              background: rateStatus === "live" ? "#34c759" : "#ff9500" }} />
            <span style={{ fontSize: 11, fontWeight: 600 }}>${"1"}=₱{usdRate.toFixed(1)}</span>
          </div>
        </header>

        <main style={{ maxWidth: 600, margin: "0 auto", padding: "36px 16px 100px" }}>
            ...
            <div key="in">
              <div className="fu" style={{ marginBottom: 36 }}>
                ...
                <p style={{ fontSize: 12, color: "#aeaeb2", marginTop: 6 }}>
                  USD auto-converts · VAT applied per RA 12023 · Data saved on device
                </p>
              </div>
              ...
              <Block title="Income" cls="fu fu1">
                ...
                {extras.map((e, i) => (
                  <DualRow key={i} label={e.label} sub="Extra"
                    amount={e.amount} currency={e.currency} rate={usdRate} readOnly indent />
                ))}
                {showExtra ? (
                  <div style={{ padding: "10px 0" }}>
                    <input placeholder="Label" value={newExtra.label}
                      onChange={e => setNewExtra({ ...newExtra, label: e.target.value })}
                      style={{ ...tiS, width: "100%", marginBottom: 8 }} />
                    <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                      <CurrencyToggle value={newExtra.currency} onChange={c => setNewExtra({ ...newExtra, currency: c })} />
                      <AmountInput value={newExtra.amount} currency={newExtra.currency} onChange={v => setNewExtra({ ...newExtra, amount: v })} />
                ...
                <div style={{ display: "flex", gap: 8, padding: "10px 4px" }}>
                  {[15000, 18000].map(v => (
                    <button key={v} onClick={() => setSavingsGoal(String(v))}
                      ...
                      style={{ padding: "6px 12px", borderRadius: 20, border: "1px solid #d0d0d0",
              ...
              <Block title="General Expenses" cls="fu fu3">
                {EXPENSE_CATS.map(c => (
                  <DualRow key={c} label={c}
                    amount={expenses[c]?.amount} currency={expenses[c]?.currency || "PHP"} rate={usdRate}
              ...
              <div className="fu fu4" style={{ marginBottom: 20 }}>
                ...
                {totalTax > 0 && (
                  <div style={{ display: "flex", alignItems: "center", gap: 8,
                    ...
                    <span style={{ fontSize: 12, color: "#7a5c00", fontWeight: 500 }}>
                      ⚠ {fmtPHP(totalTax)} total VAT (RA 12023)
                    </span>
                  </div>
                )}
                <div style={{ background: "#fff", border: "1px solid #e0e0e0", borderRadius: 16,
                  ...
                  {subs.length === 0 && !showSubPicker && !showCustomSub && (
                    <p style={{ fontSize: 13, color: "#c7c7cc", padding: "12px 0", textAlign: "center" }}>
                      No subscriptions yet.
                    </p>
                  )}
                  {subRows.map((s, i) => (
                    <div key={i} className="sub-row-wrap" style={{ padding: "12px 0", borderBottom: "1px solid #f5f5f7" }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          ...
                          <div style={{ display: "flex", gap: 6, marginTop: 3, alignItems: "center" }}>
                            ...
                            {s.taxRule && (
                              <span className={`tax-badge${s.taxRule.rate > 0 ? " active" : ""}`}>
                                {s.taxRule.label}{s.taxRule.inclusive && s.taxRule.rate > 0 ? " incl." : ""}
                              </span>
                            )}
                          </div>
                        </div>
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: 8, paddingLeft: 44, flexWrap: "wrap" }}>
                        <CurrencyToggle value={s.currency} onChange={c => updateSub(i, "currency", c)} />
                        <AmountInput value={s.amount} currency={s.currency} onChange={v => updateSub(i, "amount", v)} />
                        {parseFloat(s.amount) > 0 && (
                          <div style={{ fontSize: 11, color: "#6e6e73", lineHeight: 1.6 }}>
                            {s.currency === "USD" && <div>= {fmtPHP(s.basePHP)}</div>}
                            {s.taxed.tax > 0 && <div style={{ color: "#856404" }}>+{fmtPHP(s.taxed.tax)} VAT{s.taxRule.inclusive ? " (incl.)" : ""}</div>}
                            <div style={{ fontWeight: 700, color: "#000" }}>Total: {fmtPHP(s.taxed.total)}</div>
                          </div>
                        )}
                      </div>
                    </div>
                        ...
                        {filteredBrands.map(b => (
                          <div key={b.name} className="brand-chip" onClick={() => addBrandSub(b)}>
                            ...
                            <div>
                              ...
                              <div style={{ fontSize: 10, color: "#856404", fontWeight: 600 }}>
                  ...
                  {showCustomSub && (
                    <div style={{ marginTop: 8, padding: 12, background: "#f5f5f7", borderRadius: 12 }}>
                      ...
                      <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
                        <CurrencyToggle value={customSub.currency} onChange={c => setCustomSub({ ...customSub, currency: c })} />
                        <AmountInput value={customSub.amount} currency={customSub.currency} onChange={v => setCustomSub({ ...customSub, amount: v })} />
                        <div className="type-toggle">
                          {["Expense","Investment"].map(t => (
            ...
            <div key="dash">
              ...
              <div className="fu fu1" style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10, marginBottom: 12 }}>
                {[
                  ...
                  <div key={s.l} className="card-h" style={{ background: "#f5f5f7", borderRadius: 14,
                    padding: "14px 12px", boxShadow: "0 1px 3px rgba(0,0,0,.04)" }}>
                    <div style={{ fontSize: 10, color: "#aeaeb2", fontWeight: 600,
                      letterSpacing: "0.06em", textTransform: "uppercase", marginBottom: 8 }}>{s.l}</div>
                    <div style={{ fontSize: 16, fontWeight: 700, color: s.c, letterSpacing: "-0.02em" }}>
                ...
                <div className="fu fu1" style={{ background: "#fff8e1", border: "1px solid #ffe082",
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: 12, color: "#856404" }}>
                      {subRows.filter(s => s.taxed.tax > 0).length} subscriptions
                    </div>
                  </div>
                  <div style={{ fontSize: 20, fontWeight: 700, color: "#856404" }}>{fmtPHP(totalTax)}</div>
                </div>
              )}
              ...
              <div className="fu fu2 card-h" style={cardS}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 20 }}>
                  <svg width={140} height={140} viewBox="0 0 148 148" style={{ flexShrink: 0 }}>
                  ...
                  <div style={{ flex: 1 }}>
                    {[
                      ...
                      <div key={r.l} style={{ padding: "8px 0", borderBottom: i < 2 ? "1px solid #f5f5f7" : "none" }}>
                        <div style={{ fontSize: 10, color: "#aeaeb2", fontWeight: 600,
                    ...
                    <PieChart>
                      ...
                      <Tooltip formatter={v => [fmtPHP(v)]}
                        contentStyle={{ background: "#fff", border: "1px solid #e0e0e0",
                          borderRadius: 10, fontSize: 12, fontFamily: SF }} />
                    </PieChart>
                  </ResponsiveContainer>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0 12px" }}>
                    {pieData.map((d, i) => (
                      <div key={i} style={{ display: "flex", alignItems: "center", gap: 8,
                        ...
                        <span style={{ fontSize: 11, color: "#6e6e73", flex: 1 }}>{d.name}</span>
                        <span style={{ fontSize: 11, fontWeight: 600 }}>{fmtPHP(d.value)}</span>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <div className="fu fu5 card-h" style={cardS}>
                ...
                {totalTax > 0 && (
                  <Ins icon="◈" text={`${fmtPHP(totalTax)}/mo going to VAT. Factor this into your rates.`} color="#856404" />
                )}
                ...
                <div className="fu fu6 card-h" style={cardS}>
                  ...
                  {history.map((h, i) => (
                    <div key={i} style={{ display: "flex", justifyContent: "space-between",
                      padding: "11px 0", borderBottom: i < history.length - 1 ? "1px solid #f5f5f7" : "none" }}>
                      <div>
                        <div style={{ fontSize: 13, fontWeight: 600 }}>{h.month}</div>
                        <div style={{ fontSize: 11, color: "#aeaeb2", marginTop: 2 }}>Income {fmtPHP(h.income)}</div>
                      </div>
                      <div style={{ textAlign: "right" }}>
                        <div style={{ fontSize: 13, fontWeight: 700,
                          color: h.saved >= h.goal && h.goal > 0 ? "#34c759" : "#000" }}>
                          Saved {fmtPHP(h.saved)}
                        </div>
                        <div style={{ fontSize: 11, color: "#aeaeb2", marginTop: 2 }}>Goal {fmtPHP(h.goal)}</div>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <div key="rwy">
                    ...
                    {[10000,15000,18000,20000].map(v => (
                      <button key={v} onClick={() => setMonthlySaveTarget(v)}
                        ...
                        style={{ flex: 1, padding: "7px 0", borderRadius: 10,
                          border: "1px solid #e0e0e0", fontWeight: 500, fontSize: 11,
              ...
              <div className="fu fu2 card-h" style={{ ...cardS, background: "#000", borderColor: "#000" }}>
                ...
                <div style={{ fontSize: 38, fontWeight: 700, color: "#fff",
                  letterSpacing: "-0.04em", marginBottom: 4, lineHeight: 1 }}>
              ...
              <div className="fu fu3 card-h" style={cardS}>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: 16 }}>
                  <div>
                    <div style={{ fontSize: 12, color: "#6e6e73" }}>Saving {fmtPHP(monthlySaveTarget)}/month</div>
                    <div style={{ fontSize: 26, fontWeight: 700, letterSpacing: "-0.03em", marginTop: 2 }}>
                  ...
                  {arrivalDate() && (
                    <div style={{ textAlign: "right" }}>
                      ...
                      <div style={{ fontSize: 14, fontWeight: 700, color: "#34c759" }}>{arrivalDate()}</div>
                    </div>
                  )}
                </div>
                <div style={{ display: "grid", gridTemplateColumns: "46px 1fr 80px", gap: "0 10px", fontSize: 11 }}>
                  ...
                  {Array.from({ length: Math.min(monthsToGoal || 12, 24) }, (_, i) => {
                    ...
                    return [
                      <div key={`m${i}`} style={{ fontSize: 11, color: reached ? "#34c759" : "#6e6e73",
                        padding: "5px 0", borderBottom: "1px solid #f5f5f7", fontWeight: reached ? 700 : 400 }}>
                        {d.toLocaleString("default", { month: "short", year: "2-digit" })}
                      </div>,
                      <div key={`b${i}`} style={{ padding: "5px 0", borderBottom: "1px solid #f5f5f7", display: "flex", alignItems: "center" }}>
                        <div style={{ height: 4, background: "#f0f0f0", borderRadius: 2, overflow: "hidden", flex: 1 }}>
                          <div style={{ height: "100%", borderRadius: 2,
                            background: reached ? "#34c759" : "#000", width: `${pct}%` }} />
                        </div>
                      </div>,
                      <div key={`v${i}`} style={{ fontSize: 11, fontWeight: reached ? 700 : 500,
                        color: reached ? "#34c759" : "#3a3a3c", padding: "5px 0",
              ...
              <div className="fu fu4 card-h" style={cardS}>
                ...
                {[
                  ...
                  <div key={r.l} style={{ display: "flex", justifyContent: "space-between",
                    padding: "9px 0", borderBottom: "1px solid #f5f5f7", fontSize: 13 }}>
                    <span style={{ color: "#3a3a3c" }}>{r.l}</span>
                    <span style={{ fontWeight: 600 }}>{fmtPHP(r.v)}</span>
                  </div>
                ))}
                <div style={{ display: "flex", justifyContent: "space-between",
                  padding: "11px 0 0", fontSize: 14, fontWeight: 700 }}>
...
function DualRow({ label, sub, amount, currency, rate, onAmount, onCurrency, readOnly, indent }) {
  ...
  return (
    <div className="row" style={{ display: "flex", alignItems: "center",
      ...
      paddingLeft: indent ? 14 : 4, borderBottom: "1px solid #f5f5f7", gap: 8 }}>
      <div style={{ minWidth: 0, flex: 1 }}>
        <div style={{ fontSize: 14, fontWeight: 500 }}>{label}</div>
        {sub && <div style={{ fontSize: 11, color: "#c7c7cc", marginTop: 1 }}>{sub}</div>}
        {currency === "USD" && parseFloat(amount) > 0 && (
          <div style={{ fontSize: 11, color: "#aeaeb2", marginTop: 1 }}>= {fmtPHP(php)}</div>
        )}
      </div>
      {!readOnly ? (
        ...
        <div style={{ fontSize: 13, fontWeight: 600, color: "#6e6e73" }}>{fmtPHP(php)}</div>
      )}
    </div>
  );
}
...
function AmountInput({ value, currency, onChange, large }) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", background: "#f5f5f7",
      ...
      <span style={{ padding: "7px 6px 7px 9px", fontSize: 12, color: "#aeaeb2",
        fontWeight: 500, borderRight: "1px solid #e5e5e5" }}>{sym}</span>
      <input type="number" placeholder="0" value={value}
        onChange={e => onChange && onChange(e.target.value)}
        style={{ background: "transparent", border: "none", outline: "none",
          color: "#000", fontSize: large ? 15 : 13, fontWeight: large ? 700 : 600,
          padding: "7px 7px", width: large ? 100 : 74, textAlign: "right",
  ...
  return (
    <button onClick={onClick} className={outline ? "ghost cta" : "cta"}
      style={{ background: outline ? "transparent" : "#000", color: outline ? "#6e6e73" : "#fff",
        border: outline ? "1px solid #d0d0d0" : "none", padding: "7px 12px",
...
function Chip({ label, value, color, bold }) {
  return (
    <div style={{ display: "flex", gap: 5, alignItems: "center",
      ...
      <span style={{ fontSize: 10, color: "#aeaeb2", fontWeight: 500 }}>{label}</span>
      <span style={{ fontSize: 11, fontWeight: bold ? 700 : 600, color: color || "#000" }}>{value}</span>
    </div>
  );
}
...
function Ins({ icon, text, color = "#6e6e73" }) {
  return (
    <div style={{ display: "flex", gap: 10, padding: "9px 0", borderBottom: "1px solid #f5f5f7" }}>
      <span style={{ fontSize: 12, color, flexShrink: 0, width: 16, textAlign: "center" }}>{icon}</span>
      <span style={{ fontSize: 12, color: color !== "#6e6e73" ? "#000" : "#6e6e73", lineHeight: 1.5 }}>{text}</span>
    </div>
  );
}
...
const h1    = { fontSize: 32, fontWeight: 700, letterSpacing: "-0.03em", color: "#000", marginTop: 8, lineHeight: 1.1 };
const cardS = { background: "#fff", border: "1px solid #e0e0e0", borderRadius: 18,
                padding: "18px", marginBottom: 12, boxShadow: "0 1px 4px rgba(0,0,0,.04)" };
...
const tiS   = { background: "#fff", border: "1px solid #e0e0e0", borderRadius: 8,
                padding: "7px 11px", fontSize: 13, outline: "none", color: "#000",
                fontFamily: `-apple-system, BlinkMacSystemFont, "Helvetica Neue", sans-serif` };
